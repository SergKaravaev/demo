...
public interface UserControllerDocumentation {
    ...
    ResponseEntity<UserResponseDto> updateUser(UUID userId, UserRequestDto userRequestDto);
    ...
    @Operation(summary = "Check if a user exists",
            description = "Checks whether a user with the specified ID exists in the system.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "User exists", content = @Content),
            @ApiResponse(responseCode = "404", description = "User not found", content = @Content)
    })
    ResponseEntity<Void> checkUserExists(UUID userId);
}
